...
void DeepboxAudioProcessor::processBlock (AudioBuffer<float>& buffer, MidiBuffer& midiMessages)
{
    ...
    if(std::find(approved_buffer_sizes.begin(), approved_buffer_sizes.end(), samples_Per_Block) != approved_buffer_sizes.end()){
        ...
        if(onset_detected && db > current_onset_threshold && onset_below_floor_threshold){
            ...
            const fdeep::tensor input(fdeep::tensor_shape(audio_feature_size, 1), audio_features);
...
void DeepboxAudioProcessor::initialiseSynth()
{
    ...
    std::unique_ptr<AudioFormatReader> readerKickDrum(formatManager.createReaderFor(File::getSpecialLocation( File::SpecialLocationType::currentApplicationFile).getChildFile("Contents/Resources/wavs/bassdrum.wav")));
    std::unique_ptr<AudioFormatReader> readerSnareDrum(formatManager.createReaderFor(File::getSpecialLocation( File::SpecialLocationType::currentApplicationFile).getChildFile("Contents/Resources/wavs/snaredrum.wav")));
    std::unique_ptr<AudioFormatReader> readerHiHat(formatManager.createReaderFor(File::getSpecialLocation( File::SpecialLocationType::currentApplicationFile).getChildFile("Contents/Resources/wavs/hihat.wav")));
